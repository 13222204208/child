<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="图片" prop="icon" width="180">
            <template #default="scope">
            <el-image style="width: 50px;" :src="basePath+scope.row.icon" />
            </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBoardFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="图标:"  prop="icon" >
          <el-image style="width: 50px;" :src="basePath+formData.icon" />
          <br />
...
const getTableData = async() => {
  const table = await getBoardList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteBoardByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBoardFunc = async(row) => {
    const res = await findBoard({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBoardFunc = async (row) => {
    const res = await deleteBoard({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
