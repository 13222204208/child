  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateMenubarFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getMenubarList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteMenubarByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateMenubarFunc = async(row) => {
    const res = await findMenubar({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.remenubar
...
// 删除行
const deleteMenubarFunc = async (row) => {
    const res = await deleteMenubar({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createMenubar(formData.value)
                  break
                case 'update':
                  res = await updateMenubar(formData.value)
                  break
                default:
                  res = await createMenubar(formData.value)
                  break
