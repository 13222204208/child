...
const getTableData = async() => {
  const table = await getScanList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteScanByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateScanFunc = async(row) => {
    const res = await findScan({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rescan
...
// 删除行
const deleteScanFunc = async (row) => {
    const res = await deleteScan({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createScan(formData.value)
                  break
                case 'update':
                  res = await updateScan(formData.value)
                  break
                default:
                  res = await createScan(formData.value)
                  break
